...
bool evaluate(vector<long long>& numbers, long long targetValue, size_t currentIndex, long long currentResult) {
    ...
    long long concatenatedResult = currentResult;
    concatenatedResult = concatenatedResult * pow(10, to_string(numbers[currentIndex]).length()) + numbers[currentIndex];
    
    if (evaluate(numbers, targetValue, currentIndex + 1, concatenatedResult)) return true;
